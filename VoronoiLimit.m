...
try
    ...
    for ij=1:length(C)
        if any(C{ij}==1)
            ...
            if (length(ixa)==2 || length(ixb)==2) && ~isequal(ixa,ixb)
    ...
    %     plot(extremebounds(1:2),[extremebounds(3) extremebounds(3)],'--k')
    %     plot(extremebounds(1:2),[extremebounds(4) extremebounds(4)],'--k')
    %     plot([extremebounds(1) extremebounds(1)],extremebounds(3:4),'--k')
    %     plot([extremebounds(2) extremebounds(2)],extremebounds(3:4),'--k')
    %     plot(exb_vertices(:,1),exb_vertices(:,2),'*k')
    ...
    for ij=1:length(C)
        if sum(allVixinp(C{ij}))~=length(C{ij})
            ...
            while kk==0
                ...
                if any(intersect) %expand outer points
                    poly0=[V(C{ij},1),V(C{ij},2)];
                    diagnostics=zeros(3,4^3); % 1) convexhull area | 2) does not contain other centroids
                    polylog=cell(4^3,1);
                    ctr=0;
                    for iv1=1:4     % four extreme boundary vertices
                        for iv2=1:4 % four extreme boundary vertices
                            for iv3=0:4 % Optional (iv3=0) additional points (iv3=1:4)
                                ctr=ctr+1;
                                run=1;
                                %check all possible variations of connections
                                %between open ends and extreme boundary limits
